...
public final class ReportPathUtils {
	...
	public static String createExtentReportPath() {
		if (ConfigLoader.getInstance().getOverrideReports().equalsIgnoreCase(ExtentReportConstant.NO)) {
			/**
			 * Report name ->
			 *
			 ...
			 */
			return ExtentReportConstant.EXTENT_REPORT_FOLDER_PATH + OSInfoUtils.getOSInfo() + "_" + DateUtils.getCurrentDate() + "_"
					+ ExtentReportConstant.EXTENT_REPORT_NAME;

		} else {
			return ExtentReportConstant.EXTENT_REPORT_FOLDER_PATH + ExtentReportConstant.EXTENT_REPORT_NAME;
		}
	}
}
